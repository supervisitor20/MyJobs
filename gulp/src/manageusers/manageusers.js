...
const Role = React.createClass({
    ...
    if(action == 'Edit'){
      $.get('/manage-users/api/roles/' + this.props.params.roleId, function(results) {
        if (this.isMounted()) {
          ...
          let roleObject = results[this.props.params.roleId];

          let roleName = roleObject.role.name;

          const availableUsersUnformatted = JSON.parse(roleObject.users.available);
          let availableUsers = availableUsersUnformatted.map(function(obj){
             let user = {};
             user.id = obj.pk;
          ...
          let assignedUsers = assignedUsersUnformatted.map(function(obj){
             let user = {};
             user.id = obj.pk;
             user.name = obj.fields.email;
             return user;
          ...
          let availableActivities = availableActivitiesUnformatted.map(function(obj){
            let activity = {};
            activity.id = obj.pk;
          ...
          let assignedActivities = assignedActivitiesUnformatted.map(function(obj){
             let activity = {};
             activity.id = obj.pk;
             activity.name = obj.fields.name;
             return activity;
  ...
  render() {
    ...
    let roleNameHelp = this.state.roleNameHelp;

    let apiResponseHelp = this.state.apiResponseHelp;

    let activitiesMultiselectHelp = this.state.activitiesMultiselectHelp;
...
const ActivitiesMultiselect = React.createClass({
  ...
  _onDeselect(deselectedOptions) {
    let assignedActivities = this.state.assignedActivities.slice()
  ...
  render() {
    let {assignedActivities, availableActivities} = this.state
...
const UsersMultiselect = React.createClass({
  ...
  _onDeselect(deselectedOptions) {
    let assignedUsers = this.state.assignedUsers.slice()
    deselectedOptions.forEach(option => {
  ...
  render() {
    let {assignedUsers, availableUsers} = this.state
...
const RolesMultiselect = React.createClass({
  ...
  _onDeselect(deselectedOptions) {
    let assignedRoles = this.state.assignedRoles.slice();
    deselectedOptions.forEach(option => {
  ...
  render() {
    let {assignedRoles, availableRoles} = this.state
...
const User = React.createClass({
    ...
    if(action == 'Edit'){
      $.get('/manage-users/api/users/' + this.props.params.userId, function(results) {
        ...
        if (this.isMounted()) {
          ...
          let userObject = results[this.props.params.userId];

          let userEmail = userObject.email;

          let availableRolesUnformatted = JSON.parse(userObject.roles.available);
          let availableRoles = availableRolesUnformatted.map(function(obj){
             let role = {};
             role.id = obj.pk;
          ...
          let assignedRolesUnformatted = JSON.parse(userObject.roles.assigned);
          let assignedRoles = assignedRolesUnformatted.map(function(obj){
             let role = {};
             role.id = obj.pk;
             role.name = obj.fields.name;
             return role;
  ...
  render() {
    ...
    let userEmailHelp = this.state.userEmailHelp;
    let roleMultiselectHelp = this.state.roleMultiselectHelp;
    let apiResponseHelp = this.state.apiResponseHelp;
