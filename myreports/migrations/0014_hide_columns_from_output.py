...
class Migration(DataMigration):
    def get_config(self, orm, report_type):
        return (
            orm["myreports.ReportTypeDataTypes"].objects
            .get(
                report_type__report_type=report_type,
                data_type__data_type="unaggregated")).configuration

    def iter_col_configs(self, orm):
        config_contacts = self.get_config(orm, 'contacts')
        config_commrecs = self.get_config(orm, 'communication-records')

        config_partner_tags = (
            config_contacts.configurationcolumn_set.filter(
                column_name='partner_tags'))
        commrec_partner_tags = (
            config_commrecs.configurationcolumn_set.filter(
                column_name='partner_tags'))
        commrec_contact_tags = (
            config_commrecs.configurationcolumn_set.filter(
                column_name='contact_tags'))
    ...
    def forwards(self, orm):
        ...
        for col_config in self.iter_col_configs(orm):
            col_config.filter_only = True
            col_config.save()
