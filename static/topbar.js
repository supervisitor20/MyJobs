...
function process_reload() {
  ...
  var app_names = ["candidates", "prm", "posting"];
  var new_href = window.location.protocol + "//" + window.location.host;
  var app_name;
  var i;
  ...
  for (i = 0; i < app_names.length; i++) {
    ...
    if (path.indexOf(app_name) !== -1) {
      if (app_name === "candidates") {
        ...
        } else if (href.indexOf("purchased-jobs") !== -1) {
          window.location.href = new_href + "/posting/purchased-jobs/";
        } else {
          window.location.href = new_href + "/posting/all";
        }
