...
class ModelTests(MyJobsBase):
    ...
    def test_productgrouping_add_delete(self):
        ...
        order = ProductOrder.objects.create(product=self.product,
                                            group=grouping)
        grouping_products = grouping.products.all().values_list(
            'pk', flat=True)
    ...
    def test_invoice_filter_by_sites(self):
        ...
        for x in range(8800, 8815):
            ...
            for y in range(1, 5):
                ...
                purchase = OfflinePurchaseFactory(
                    owner=self.company, created_by=cu)
                OfflineProduct.objects.create(
                    product=product, offline_purchase=purchase)
        ...
        for x in range(8800, 8815):
            ...
            for y in range(1, 5):
                product = ProductFactory(
                    package=site_package, owner=self.company)
                PurchasedProductFactory(product=product, owner=self.company)
    ...
    def test_product_filter_by_site_multiple_sites(self):
        ...
        # both jobs.
        count = Product.objects.filter_by_sites(
            [site_in_both_packages]).count()
        self.assertEqual(count, 2)
    ...
    def test_purchasedproduct_filter_by_site_multiple_sites(self):
        ...
        single_site_product = ProductFactory(
            package=single_site_package, owner=self.company)
        PurchasedProductFactory(
            product=single_site_product, owner=self.company)

        both_sites_product = ProductFactory(
            package=both_sites_package, owner=self.company)
    ...
    def test_sitepackage_filter_by_site_multiple_sites(self):
        ...
        count = objs.count()
        self.assertEqual(count, 2)
        count = Package.objects.filter_by_sites(
            [site_in_both_packages]).count()
        self.assertEqual(count, 2)
    ...
    def test_offlinepurchase_filter_by_site_multiple_sites(self):
        ...
        single_site_product = ProductFactory(
            package=single_site_package, owner=self.company)
        single_site_purchase = OfflinePurchaseFactory(
            owner=self.company, created_by=cu)
        ...
        both_sites_product = ProductFactory(
            package=both_sites_package, owner=self.company)
        both_sites_purchase = OfflinePurchaseFactory(
            owner=self.company, created_by=cu)
    ...
    def test_invoice_from_offlinepurchase_filter_by_site_multiple_sites(self):
        ...
        single_site_purchase = OfflinePurchaseFactory(
            owner=self.company, created_by=cu)
        OfflineProduct.objects.create(
            product=single_site_product, offline_purchase=single_site_purchase)
        ...
        both_sites_purchase = OfflinePurchaseFactory(
            owner=self.company, created_by=cu)
        OfflineProduct.objects.create(
            product=both_sites_product, offline_purchase=both_sites_purchase)
    ...
    def test_invoice_from_purchasedproduct_filter_by_site_multiple_sites(self):
        ...
        # Confirm that filtering by the site the site that has both products
        #  gets both jobs.
        count = Invoice.objects.filter_by_sites(
            [site_in_both_packages]).count()
        self.assertEqual(count, 2)
