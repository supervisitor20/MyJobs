...
def swap_names(list_old, list_new):
    for i in range(0, len(list_old)):
        owfs = OutreachWorkflowState.objects.get(state=list_old[i])
        owfs.state = list_new[i]
        owfs.save()
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        swap_names(['new','waiting for info','complete'],
                   ['New','More Info Needed','Complete'])

    def backwards(self, orm):
        swap_names(['New','More Info Needed','Complete'],
                   ['new','waiting for info','complete'])
