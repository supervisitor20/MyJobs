...
class Migration(SchemaMigration):
        ...
        for record in records:
            ...
            for email in emails:
                with open("tag_changes.txt", "a+") as fd:
                    ...
                    fd.write("Adding tags to contact record %s.\n" % record.pk)
                    fd.write(
                        "Tags to be added from Partner Saved Search %s:\n\t- %s\n" % (
                            email.pk, "\n\t- ".join(
                                tag.name for tag in email.tags.all())))
    ...
    models = {
        ...
        u'mysearches.partnersavedsearch': {
            ...
            'created_by': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'created_by'", 'null': 'True', 'on_delete': 'models.SET_NULL', 'to': u"orm['myjobs.User']"}),
